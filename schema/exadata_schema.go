...
func ValidateExadata(raw []byte) error {
	schemaLoader, err := loadExadataSchema()
	if err != nil {
		return nil
	}
...
func loadExadataSchema() (*gojsonschema.Schema, error) {
	...
	for i := range schemas {
		...
		if err := sl.AddSchemas(jl); err != nil {
			return nil, utils.NewError(err, "Wrong exadata schema: [%s]", schemas[i])
		}
	}
	...
	exaSchema, err := sl.Compile(role)
	if err != nil {
		return nil, utils.NewError(err, "Wrong exadata schema: can't load or compile it")
	}

	return exaSchema, nil
}
