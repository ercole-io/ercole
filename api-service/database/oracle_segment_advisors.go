		...
		mu.MAPipeline(
			...
			bson.M{
				"$project": bson.M{
					...
					"segmentsSize":   "$features.oracle.database.databases.pdbs.oracledatabasepdbsize.segmentsSize",
					...
					"retrieve": bson.M{"$cond": bson.A{
						bson.M{"$eq": bson.A{"$features.oracle.database.databases.pdbs.oracledatabasepdbsize.segmentsSize", 0}},
						0.0,
						bson.M{"$divide": bson.A{"$features.oracle.database.databases.pdbs.segmentAdvisors.reclaimable",
							"$features.oracle.database.databases.pdbs.oracledatabasepdbsize.segmentsSize"}}}},
				},
			},
			bson.M{
				"$project": bson.M{
					...
					"recommendation": 1,
					"pdbName":        1,
					"retrieve":       bson.M{"$round": bson.A{"$retrieve", 4}}},
			},
			mu.APOptionalSortingStage(sortBy, sortDesc),
		))
	if err != nil {
