...
func (m *MongodbSuite) TestGetTotalDatabaseDatafileSizeStats() {
	defer m.db.Client.Database(m.dbname).Collection("hosts").DeleteMany(context.TODO(), bson.M{})
	...
	m.T().Run("should_filter_out_by_environment", func(t *testing.T) {
		out, err := m.db.GetTotalDatabaseDatafileSizeStats("", "FOOBAR", utils.MAX_TIME)
		m.Require().NoError(err)
	...
	m.T().Run("should_filter_out_by_older_than", func(t *testing.T) {
		out, err := m.db.GetTotalDatabaseDatafileSizeStats("", "", utils.MIN_TIME)
		m.Require().NoError(err)
	...
	m.T().Run("should_return_correct_results", func(t *testing.T) {
		out, err := m.db.GetTotalDatabaseDatafileSizeStats("", "", utils.MAX_TIME)
		m.Require().NoError(err)
