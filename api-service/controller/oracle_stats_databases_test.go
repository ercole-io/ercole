...
func TestGetOracleDatabaseLicenseComplianceStatusStats_Success(t *testing.T) {
	mockCtrl := gomock.NewController(t)
	defer mockCtrl.Finish()
	as := NewMockAPIServiceInterface(mockCtrl)
	ac := APIController{
	...
	as.EXPECT().
		GetOracleDatabaseLicenseComplianceStatusStats("Italy", "TST", utils.P("2020-06-10T11:54:59Z")).
		Return(expectedRes, nil)

	rr := httptest.NewRecorder()
	handler := http.HandlerFunc(ac.GetOracleDatabaseLicenseComplianceStatusStats)
...
func TestGetOracleDatabaseLicenseComplianceStatusStats_FailUnprocessableEntity(t *testing.T) {
	mockCtrl := gomock.NewController(t)
	defer mockCtrl.Finish()
	as := NewMockAPIServiceInterface(mockCtrl)
	ac := APIController{
...
func TestGetOracleDatabaseLicenseComplianceStatusStats_FailInternalServerError(t *testing.T) {
	mockCtrl := gomock.NewController(t)
	defer mockCtrl.Finish()
	as := NewMockAPIServiceInterface(mockCtrl)
	ac := APIController{
	...
	as.EXPECT().
		GetOracleDatabaseLicenseComplianceStatusStats("", "", utils.MAX_TIME).
		Return(nil, aerrMock)
