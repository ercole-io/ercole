...
func (ctrl *APIController) setupAdminRoutes(router *mux.Router) {
	...
	router.HandleFunc(fmt.Sprintf("%s/{username}/{provider}", userGroup), middleware.Admin(ctrl.UpdateUser)).Methods("PUT")
	router.HandleFunc(fmt.Sprintf("%s/{username}/{provider}", userGroup), middleware.Admin(ctrl.RemoveUser)).Methods("DELETE")
