...
func TestListOracleDatabasePoliciesAudit(t *testing.T) {
	mockCtrl := gomock.NewController(t)
	defer mockCtrl.Finish()
	db := NewMockMongoDatabaseInterface(mockCtrl)
	as := APIService{
	...
	expected := []dto.OraclePoliciesAuditListResponse{
		{
			Hostname:                "hostname",
			DbName:                  "dbname",
			Flag:                    "green",
	...
	db.EXPECT().ListOracleDatabasePoliciesAudit().Return(expected, nil)

	res, err := as.ListOracleDatabasePoliciesAudit()
