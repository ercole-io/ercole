...
func TestGetOracleDatabaseLicenseComplianceStatusStats_Success(t *testing.T) {
	...
	db.EXPECT().GetOracleDatabaseLicenseComplianceStatusStats(
		"Italy", "PROD", utils.P("2019-12-05T14:02:03Z"),
	).Return(expectedRes, nil).Times(1)

	res, err := as.GetOracleDatabaseLicenseComplianceStatusStats(
		"Italy", "PROD", utils.P("2019-12-05T14:02:03Z"),
	)
...
func TestGetOracleDatabaseLicenseComplianceStatusStats_Fail(t *testing.T) {
	mockCtrl := gomock.NewController(t)
	defer mockCtrl.Finish()
	db := NewMockMongoDatabaseInterface(mockCtrl)
	as := APIService{
