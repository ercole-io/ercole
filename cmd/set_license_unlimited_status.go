...
func init() {
	setLicenseUnlimitedStatusCmd := &cobra.Command{
		Use:   "set-license-unlimited",
		Short: "Set the unlimited status of a license",
		Long:  `Set the unlimited status of a license`,
		Run: func(cmd *cobra.Command, args []string) {
			...
			for i := 0; i < len(args); i += 2 {
				...
				if err != nil {
					fmt.Fprintf(os.Stderr, "Unable to parse the unlimited status %q, err: %v\n", args[i+1], err)
					os.Exit(1)
				}
				...
				if err != nil {
					fmt.Fprintf(os.Stderr, "Failed to set unlimited status of license %q: %v\n", name, err)
					os.Exit(1)
				} else if resp.StatusCode < 200 || resp.StatusCode > 299 {
					out, _ := ioutil.ReadAll(resp.Body)
					defer resp.Body.Close()
					fmt.Fprintf(os.Stderr, "License: %s unlimited status: %d Cause: %s\n", name, resp.StatusCode, string(out))
					os.Exit(1)
				}
			}
		},
	...
	apiCmd.AddCommand(setLicenseUnlimitedStatusCmd)
}
